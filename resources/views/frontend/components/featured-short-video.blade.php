@php
    $block  = $featured_short_videos ?? [];
    $widget = $block['widget'] ?? null;
    $items  = collect($block['items'] ?? []);
@endphp
@if($widget && $items->isNotEmpty())
    <div>
        <div class="pb-4 md:container home-hero h-auto h-[300px]  overflow-hidden">
            <div class="block-title flex items-center space-x-1 p-4 md:px-0">
                <div class="flex flex-1">
                    <h4 class="p-icon-1 font-semibold line-clamp-1 text-base md:text-[20px]">
                        {{$widget->name}}
                    </h4>
                </div>
            </div>
            <div class="md:-ml-1 h-full">
                <div class="relative flex h-full w-full items-center">
                    <div
                        class=" h-[300px] swiper swiper-horizontal custom-swiper-navigation w-full !mx-0 overflow-hidden !px-4 md:!px-0"
                    >
                        <div class="swiper-wrapper h-full">
                            @foreach($items as $item)
                                <div class="swiper-slide !w-[178px] h-full">
                                    <div class=" h-full relative">
                                        <div class="product-card h-full">
                                            <div
                                                class="flex h-full flex-col overflow-hidden rounded-lg border bg-white shadow-sm"
                                            >
                                                <div class="product-card-image relative h-full">
                                                    <a href="https://www.youtube.com/embed/{{$item->youtube_id}}"
                                                       class="block h-full w-full">
                                                        <img
                                                            class="absolute inset-0 h-full w-full object-cover"
                                                            src="{{ $item->image  ? asset('storage/' . $item->image) : asset('frontend/assets/image/video/test.png') }}"
                                                            alt="{{$item->name ?? ''}}"
                                                            loading="lazy"
                                                            decoding="async"
                                                        />
                                                        <span
                                                            class="absolute bottom-0 left-0 h-[26px] w-full
