        ...
        <div class="product-card flex h-full flex-col">
            <div class="flex h-full flex-1 flex-col overflow-hidden rounded-lg border bg-white pb-[1px] shadow-sm">
                <div class="product-card-image">
                    <a href="{{ router_link('products', $product->id) }}">
                        <img
                            ...
                            src="{{ $product->image ? asset('storage/'.$product->image) : asset('backend/img/not-found.jpg') }}"
                            alt="{{ $product->name ?? '' }}"
                            loading="lazy"
                            width="500"
                            height="500"
                ...
                <div class="flex flex-1 flex-col p-2 font-medium">
                    ...
                    <div class="flex flex-1 flex-col justify-end"><div class="flex items-end justify-center"><button data-size="sm" type="button" class="relative flex justify-center outline-none font-semibold text-white bg-primary-500 border-0 hover:bg-primary-600 focus:ring-primary-300 w-full text-sm px-4 py-2 h-9 items-center rounded-lg mt-2"><span>Chọn sản phẩm</span></button></div></div>
                </div>
            </div>
        </div>
    </div>
