        ...
        <div class="col-md-12">
            <p style="margin: 0; padding: 5px 0;">
                <span class="text-semibold">
                    ...
                    -
                    {{ ($customers->currentPage() - 1) * $customers->perPage() + $customers->count() }}
                </span>
                trong
                <span class="text-semibold">{{ $customers->total() }}</span> bản ghi.
            </p>
                ...
                <div class="navbar navbar-default navbar-xs fillter padding-0">
                    <ul class="nav navbar-nav no-border visible-xs-block">
                        <li>
                            ...
                            <a href="{{ route('admin.customers.create') }}" style="float: right;"
                ...
                <div class="table-responsive" style="min-height:400px;">
                    <table class="table table-bordered table-striped">
                        ...
                        @if($customers && $customers->count())
                            @foreach($customers as $stt => $customer)
                                <tr>
                                    <td class="text-center">
                                        {{ ($customers->currentPage() - 1) * $customers->perPage() + $stt + 1 }}
                                    </td>
                                    <td>{{$customer->name ?? 'Không có dữ liệu'}}</td>
                                    <td>{{$customer->email ?? 'Không có dữ liệu'}}</td>
                                    <td>{{$customer->phone ?? 'Không có dữ liệu'}}</td>
                                    <td>
                                        {{ $customer->birthday ? \Carbon\Carbon::parse($customer->birthday)->format('d/m/Y') : 'Không có dữ liệu' }}
                                    </td>
                                    <td>{{$customer->address ?? 'Không có dữ liệu'}}</td>
                                    <td class="text-center">
                                        @include('backend.components.status', ['field' => 'status',  'model' => $model,  'modelId' => $customer->id, 'value' => $customer->status,  ])
                                    </td>
                                    <td class="text-center">
                                        <ul class="icons-list">
                                            ...
                                            <li>
                                                <form action="{{ route('admin.customers.delete', $customer->id) }}"
                                                      ...
                                                      onsubmit="return confirm('Bạn có chắc chắn muốn xoá {{ $customer->name }}?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
