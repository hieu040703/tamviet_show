        ...
        <div class="col-md-12">
            <p style="margin: 0; padding: 5px 0;">
                <span class="text-semibold">
                    {{ ($post_catalogues->currentPage() - 1) * $post_catalogues->perPage() + 1 }}
                    -
                    {{ ($post_catalogues->currentPage() - 1) * $post_catalogues->perPage() + $post_catalogues->count() }}
                </span>
                trong
                <span class="text-semibold">{{ $post_catalogues->total() }}</span> bản ghi.
            </p>
            ...
            <div class="panel panel-flat">
                ...
                <div class="navbar navbar-default navbar-xs fillter padding-0">
                    <ul class="nav navbar-nav no-border visible-xs-block">
                        <li>
                            ...
                            <a href="{{ route('admin.post_catalogues.create') }}" style="float: right;"
                            ...
                            <a style="float: right;" class="color-black btn" data-popup="tooltip" title="Huỷ lọc"
                               href="{{ route('admin.post_catalogues.index') }}">
                        ...
                        <form action="" method="GET">
                            ...
                            <div class="navbar-right hidden-xs">
                                ...
                                <a class="btn text-primary" data-popup="tooltip"
                                   title="{{ __('Create New Post Catalogue') }}"
                                   href="{{ route('admin.post_catalogues.create') }}">
                                    <i class="icon-plus3 position-left"></i>
                                </a>
                            </div>
                        </form>
                ...
                <div class="table-responsive" style="min-height:400px;">
                    <table class="table table-bordered table-striped">
                        ...
                        <tbody>
                        @if($post_catalogues && $post_catalogues->count())
                            @foreach($post_catalogues as $stt => $post_catalogue)
                                <tr>
                                    <td class="text-center">
                                        {{ ($post_catalogues->currentPage() - 1) * $post_catalogues->perPage() + $stt + 1 }}
                                    </td>
                                    <td>
                                        {{ str_repeat('|----', (($post_catalogue->level > 0)?($post_catalogue->level - 1):0)).$post_catalogue->name }}
                                    </td>
                                    <td class="text-center">
                                        @include('backend.components.status', ['field' => 'status',  'model' => $model,  'modelId' => $post_catalogue->id, 'value' => $post_catalogue->status,  ])
                                    </td>
                                    <td class="text-center">
                                        @include('backend.components.status', ['field' => 'is_featured',  'model' => $model,  'modelId' => $post_catalogue->id, 'value' => $post_catalogue->is_featured,  ])
                                    </td>
                                    <td class="text-center">
                                        <ul class="icons-list">
                                            <li style="margin-right:10px;">
                                                <a href="{{ route('admin.post_catalogues.edit', $post_catalogue->id) }}"
                                                   class="text-blue">
                                                    <i class="icon-pencil7"></i>
                                                </a>
                                            </li>
                                            <li>
                                                <form action="{{ route('admin.post_catalogues.delete', $post_catalogue->id) }}"
                                                      method="POST" style="display:inline;"
                                                      onsubmit="return confirm('Bạn có chắc chắn muốn xoá {{ $post_catalogue->name }}?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                    ...
                    <div style="padding:10px 15px;text-align:center;">
                        {{ $post_catalogues->appends(request()->query())->links('pagination::bootstrap-4') }}
                    </div>
                </div>
            </div>
        </div>
