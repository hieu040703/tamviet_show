    ...
    <div class="row">
        <div class="col-md-12">
            <p style="margin: 0; padding: 5px 0;">
                ...
                trong
                <span class="text-semibold">{{ $posts->total() }}</span> bản ghi.
            </p>
                ...
                <div class="navbar navbar-default navbar-xs fillter padding-0">
                    <ul class="nav navbar-nav no-border visible-xs-block">
                        <li>
                            ...
                            <a href="{{ route('admin.posts.create') }}" style="float: right;"
                ...
                <div class="table-responsive" style="min-height:400px;">
                    <table class="table table-bordered table-striped">
                        ...
                        @if($posts && $posts->count())
                            @foreach($posts as $stt => $post)
                                <tr>
                                    <td class="text-center">
                                        {{ ($posts->currentPage() - 1) * $posts->perPage() + $stt + 1 }}
                                    </td>
                                    <td>
                                        {{ str_repeat('|----', (($post->level > 0)?($post->level - 1):0)).$post->name }}
                                    </td>
                                    <td class="text-center">
                                        @include('backend.components.status', ['field' => 'status',  'model' => $model,  'modelId' => $post->id, 'value' => $post->status,  ])
                                    </td>
                                    <td class="text-center">
                                        @include('backend.components.status', ['field' => 'is_featured',  'model' => $model,  'modelId' => $post->id, 'value' => $post->is_featured,  ])
                                    </td>
                                    <td class="text-center">
                                        <ul class="icons-list">
                                            ...
                                            <li>
                                                <form action="{{ route('admin.posts.delete', $post->id) }}"
                                                      ...
                                                      onsubmit="return confirm('Bạn có chắc chắn muốn xoá {{ $post->name }}?');">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
