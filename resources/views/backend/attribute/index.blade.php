        ...
        <div class="col-md-12">
            <p style="margin: 0; padding: 5px 0;">
                <span class="text-semibold">
                    {{ ($attributes->currentPage() - 1) *$attributes->perPage() + 1 }}
                    -
                    {{ ($attributes->currentPage() - 1) *$attributes->perPage() +$attributes->count() }}
                </span>
                trong
                <span class="text-semibold">{{$attributes->total() }}</span> bản ghi.
            </p>
                ...
                <div class="navbar navbar-default navbar-xs fillter padding-0">
                    <ul class="nav navbar-nav no-border visible-xs-block">
                        <li>
                            ...
                            <a href="{{ route('admin.attributes.create') }}" style="float: right;"
                               class="btn text-right">
                                <i class="icon-plus3 position-left"></i>
                            </a>
                            <a href="{{ route('admin.attributes.index') }}" style="float: right;"
                    ...
                    <div class="navbar-collapse collapse" id="navbar-filter">
                        <form action="" method="GET">
                            ...
                            <div class="navbar-right hidden-xs">
                                ...
                                <a href="{{ route('admin.attributes.index') }}" class="btn color-black">
                                    <i class="icon-cancel-circle2 position-left"></i>
                                </a>

                                <a href="{{ route('admin.attributes.create') }}" class="btn text-primary">
                                    <i class="icon-plus3 position-left"></i>
                                </a>
                            </div>
                        </form>
                ...
                <div class="table-responsive" style="min-height:400px;">
                    <table class="table table-bordered table-striped">
                        ...
                        @if($attributes && $attributes->count())
                            @foreach($attributes as $stt => $attributeCatalogue)
                                <tr>
                                    <td class="text-center">
                                        {{ ($attributes->currentPage() - 1) *$attributes->perPage() + $stt + 1 }}
                                    </td>
                                    <td >
                                        {{$attributeCatalogue->name}}
                                    </td>
                                    <td >{{$attributeCatalogue->attributeCatalogue->name}}</td>
                                    <td class="text-center">
                                    ...
                                    <td class="text-center">
                                        <ul class="icons-list">
                                            <li style="margin-right:10px;">
                                                <a href="{{ route('admin.attributes.edit', $attributeCatalogue->id) }}"
                                                   class="text-blue">
                                            ...
                                            <li>
                                                <form
                                                    action="{{ route('admin.attributes.delete', $attributeCatalogue->id) }}"
